...
export default class Simulation {
	...
	async setup() {
		...
		this.spotLight.addToScene(scene);
		this.#defineLighting();
	...
	update() {
		...
		if (planet.objects.length > 0)
		{
			...
			dtPosition.material.uniforms.uTime.value = this.#time;
			dtVelocity.material.uniforms.uDeltaTime.value = 0.016;
			dtVelocity.material.uniforms.uTime.value = this.#time;
	...
	#defineComposer() {
		...
		const renderPass = new RenderPass(scene, this.camera.camera);
